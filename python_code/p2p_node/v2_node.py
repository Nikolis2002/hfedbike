...
if NEIGHBORHOOD == "physical":
    NODE_LIST = ["A", "B", "C"]
    ip_mapping = {"A": 99, "B": 101, "C": 102, "D": 103}
elif NEIGHBORHOOD == "docker":
    area = "region0"
    NODE_LIST = ["X", "Y", "Z", "W"]
    ip_mapping = {"X": 199, "Y": 200, "Z": 201, "W": 202}
    subzone_node = {"X": "0", "Y": "1", "Z": "2", "W": "3"}
elif NEIGHBORHOOD == "docker2":
    area = "region1"
    NODE_LIST = ["P", "Q", "R", "L"]
    ip_mapping = {"P": 204, "Q": 205, "R": 206, "L": 207}
    subzone_node = {"P": "0", "Q": "1", "R": "2", "L": "3"}
elif NEIGHBORHOOD == "docker3":
    area = "region2"
    NODE_LIST = ["A", "B", "C", "D"]
    ip_mapping = {"A": 208, "B": 209, "C": 210, "D": 211}
    subzone_node = {"A": "0", "B": "1", "C": "2", "D": "3"}

elif NEIGHBORHOOD == "docker4":
    area = "region3"
    NODE_LIST = ["E", "F", "G", "H"]
    ip_mapping = {"E": 212, "F": 213, "G": 214, "H": 215}
    subzone_node = {"E": "0", "F": "1", "G": "2", "H": "3"}
...
BASE_WEATHERS = ["Clear", "Clouds", "Fog", "Haze", "Mist", "Rain", "Snow"]
...
def online_pre_processing(
    ...
    X_1d = np.concatenate([time_feats, X_z_scored, categ_data], axis=0).astype(
        np.float32
    )
...
def data_init():
    CSV_DIR = "/data/2024_csvs/entire_year"
    weather_df = pd.read_csv(f"{CSV_DIR}/fixed_2024.csv")
    file = pd.read_csv(
        f"{CSV_DIR}/{area}_subzone{subzone_node[NODE_ID]}_bike_usage.csv"
    )
    ...
    file["hour"] = pd.to_datetime(file["hour"], errors="coerce")
    ...
    merged_df = (
        merged_df.dropna(subset=["hour"]).sort_values("hour").reset_index(drop=True)
    )
    ...
    # --- ISO week handling (keep only weeks that belong to ISO year 2024) ---
    iso = merged_df["hour"].dt.isocalendar()
    merged_df = merged_df[iso.year.eq(2024)].copy()

    merged_df["week_of_year"] = iso.week.astype(int)
...
def one_hot_encoding(data):
    ...
    BASE_WEATHERS = ["Clear", "Clouds", "Haze", "Mist", "Rain", "Snow", "Thunderstorm"]
...
def compute_z_score_values(output: pd.Series) -> dict:
    ...
    return {"number_of_samples": n, "mean": mu, "variance": variance}
...
def send_z_score(output, dealer):
    ...
    logger.info(
        "Z-Score Stats | samples=%d mean=%.6f variance=%.6f",
...
def federated_average(*weight_sets):
    ...
    if (
        len(weight_sets) == 1
        and isinstance(weight_sets[0], list)
        and all(isinstance(ws, list) for ws in weight_sets[0])
    ):
...
def send_weights(weights):
    ...
    payload_str = serialize_weights(weights)  # your existing serializer -> string
    payload_bytes = payload_str.encode("utf-8")  # bytes on the wire
    nbytes = len(payload_bytes)

    logger.info(
        "[%s] SEND -> %s (%s:%s) bytes=%d",
        NODE_ID,
        NEXT_NODE,
        NEXT_NODE_IP,
        REQ_REP_PORT,
    ...
    try:
        send_socket.send(payload_bytes)  # send bytes
        ack = send_socket.recv_string()  # ack is a string, fine
        rtt_ms = (time.perf_counter() - t0) * 1000.0

        if ack == "ACK":
            logger.info(
                "[%s] ACK <- %s rtt_ms=%.2f bytes=%d",
                NODE_ID,
                NEXT_NODE,
                rtt_ms,
                nbytes,
            ...
            logger.warning(
                "[%s] unexpected reply <- %s reply=%r rtt_ms=%.2f",
                NODE_ID,
                NEXT_NODE,
                ack,
                rtt_ms,
        ...
        rtt_ms = (time.perf_counter() - t0) * 1000.0
        logger.exception(
            "[%s] send failed -> %s after %.2fms bytes=%d",
            NODE_ID,
            NEXT_NODE,
            rtt_ms,
            nbytes,
...
def receive_weights():
    ...
    try:
        payload_bytes = recv_socket.recv()  # receive bytes
        nbytes = len(payload_bytes)

        recv_socket.send_string("ACK")

        logger.info("[%s] RECV <- %s bytes=%d (ACK sent)", NODE_ID, PREV_NODE, nbytes)

        weights_json = payload_bytes.decode(
            "utf-8"
        )  # back to string for your existing deserializer
        return deserialize_weights(weights_json)

    except Exception:
        logger.exception("[%s] receive failed <- %s", NODE_ID, PREV_NODE)
        raise
...
def send_update_and_wait_for_peers(weights):
    ...
    logger.info("[%s] -> coordinator SEND area=%r bytes=%d", NODE_ID, area, nbytes)
    dealer.send_multipart([b"", payload_bytes])
...
def split_by_week_chrono(
    df, week_col="week_of_year", test_frac=0.2, time_col="timestamp"
):
...
def fit_affine(y_true, y_pred):
    # least squares for y_true ≈ a*y_pred + b
...
def main():
    ...
    baseline_freezed = load_baseline_model()
    ...
    for idx, row in full_df.iterrows():
        ...
        if wk != current_week:
            ...
            if len(y_true_arr) > 0:
                ...
                freeze_mae = mean_absolute_error(y_true_arr, yfreeze)
                freeze_mse = mean_squared_error(y_true_arr, yfreeze)
                ...
                print(
                    f"[{NODE_ID}] Logged weekly metrics for week {current_week} → {results_file}"
                )
            else:
                print(
                    f"[{NODE_ID}] WARNING: No data collected for week {current_week}. Skipping metrics write."
                )

            week_df = pd.DataFrame(weekly_buffer)
            if not week_df.empty:
                print(
                    f"[{NODE_ID}] Running weekly training/federation on week {current_week} data (n={len(week_df)})…"
                )

                # local update of the baseline model
                last_layer = baseline.layers[-1]
                last_layer.bias.assign_add([bias])
                ...
                if NODE_ID in leaders.values():
                    ...
                    print(
                        f"[{NODE_ID}] Waiting for leader’s global update (wk {current_week})…"
                    )
                ...
                print(
                    f"[{NODE_ID}] WARNING: weekly_buffer is empty for week {current_week}. Skipping training/federation."
                )
        ...
        print(
            f"[{NODE_ID}] Hour {row_df['hour']} Week:(wk {wk}),True={true_val:.4f}, BasePred={yb:.2f}, FedPred={yf:.2f}"
        )
    ...
    if weekly_buffer:
        ...
        if len(y_true_arr) > 0:
            ...
            print(
                f"[{NODE_ID}] Logged final weekly metrics for week {current_week} → {results_file}"
            )
        else:
            print(
                f"[{NODE_ID}] WARNING: No data collected for final week {current_week}, skipping metrics."
            )
        ...
        if not week_df.empty:
            print(
                f"[{NODE_ID}] Running final training/federation on week {current_week} (n={len(week_df)})…"
            )
            output = week_df["bike_usage_next"]
            global_mean, global_sigma = send_z_score(output, glob_dealer)
            ...
            print(
                f"[{NODE_ID}] WARNING: final weekly_buffer empty, skipping final training/federation."
            )
